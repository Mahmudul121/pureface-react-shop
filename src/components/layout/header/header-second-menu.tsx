...
const HeaderSecondMenu: React.FC<MenuProps> = ({ data, className }) => {
		...
		<nav className={classNames(`headerMenu flex w-full relative`, className)}>
			{data?.map((item: any) => (
				...
				>
					<Link
						href={item.path}
						className="inline-flex items-center text-xs xl:text-base text-gray-800 text-heading px-3 xl:px-4 py-0 font-small relative group-hover:text-black"
					>
						{t(item.label)}
