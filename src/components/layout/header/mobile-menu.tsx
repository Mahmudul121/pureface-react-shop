...
import { categoriesMobileMenu, brandsMobileMenu } from "@data/static/menus";
...
//   const socials = settings?.contactDetails?.socials;
	...
	const [navbar, setNavbar] = useState("categories")
	...
	return (
		<>
			<div className="flex flex-col justify-between w-full h-full">
              ...
              <nav className="flex justify-between px-7 pb-2">
              	<a href="#categoris" onClick={() => setNavbar('categories')} className={`pb-1 ${navbar === "categories" ? "border-b-2 border-gray-800" : ""}`}>
              		Categories
              	</a>
              	<a href="#brands" onClick={() => setNavbar('brands')} className={`pb-1 ${navbar === "brands" ? "border-b-2 border-gray-800" : ""}`}>
              		Brands
              	</a>
              </nav>
              ...
              { navbar === "categories" &&
	              <ul className="mobileMenu">
	                {categoriesMobileMenu?.map((menu: any, index: number) => {
	                  const dept: number = 1;
	            ...
	            { navbar === "brands" &&
	              <ul className="mobileMenu">
	                {brandsMobileMenu?.map((menu: any, index: number) => {
	                  const dept: number = 1;
	                  const menuName: string = `sidebar-menu-${dept}-${index}`;
