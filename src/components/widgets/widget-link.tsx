...
const WidgetLink: FC<Props> = ({ className, data }) => {
  ...
  return (
    <div className={`${className}`}>
      <h4 className="text-white text-heading text-sm md:text-base xl:text-lg font-semibold mb-5 2xl:mb-6 3xl:mb-7">
        {t(`${widgetTitle}`)}
      </h4>
        ...
        {lists.map((list) => (
          ...
          >
            ...
            <Link href={list.path ? list.path : "#!"}>
              <a className="text-white transition-colors duration-200 hover:text-black">
                {t(`${list.title}`)}
              </a>
            </Link>
          </li>
