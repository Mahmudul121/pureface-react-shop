...
const WidgetSocial: FC = () => {
  ...
  return (
    <div>
      <h4 className="text-heading text-sm md:text-base xl:text-lg font-semibold mb-5 2xl:mb-6 3xl:mb-7 text-white">
      ...
      <ul className="text-xs md:text-[13px] lg:text-sm text-body flex flex-col space-y-3 lg:space-y-3.5">
        {socials?.map((social: Social, index: number) => (
          <li key={`widget-list--key${index}`} className="flex items-baseline">
            {social.icon && (
              <span className="ltr:mr-3 rtl:ml-3 relative top-0.5 lg:top-1 text-sm lg:text-base text-white">
                {getIcon({
            ...
            <Link href={social.url ? social.url : "#!"}>
              <a className="transition-colors duration-200 hover:text-black text-white">
